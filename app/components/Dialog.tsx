...
export function Dialog({ open, onClose, title, description, children, footer }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6">
      <div className="relative w-full max-w-3xl rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] p-6 shadow-[var(--shadow-soft)]">
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="mt-6 space-y-4 text-sm text-[var(--color-text-muted)]">{children}</div>
