    ...
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-1">
          <label className="text-sm font-medium text-[var(--color-text)]" htmlFor="billing-total-value">
        ...
        <div className="space-y-1">
          <label className="text-sm font-medium text-[var(--color-text)]" htmlFor="billing-cycle">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-[var(--color-text)]">Impostos e retenções</span>
          <button
            type="button"
            onClick={addTax}
        ...
        <div className="space-y-3">
          {value.taxes.map((tax, index) => (
            <div
              key={`${tax.name}-${index}`}
              className="grid gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-3 md:grid-cols-[1fr_minmax(120px,160px)_auto] md:items-center"
            >
              <input
                value={tax.name}
                onChange={(event) => updateTax(index, "name", event.target.value)}
                placeholder="ISS"
                className="w-full rounded-lg border border-[var(--color-border)] bg-transparent px-3 py-2 text-sm text-[var(--color-text)] focus:border-[var(--color-primary)] focus:outline-none"
              />
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-full rounded-lg border border-[var(--color-border)] bg-transparent px-3 py-2 text-sm text-[var(--color-text)] focus:border-[var(--color-primary)] focus:outline-none"
                />
                <span className="text-xs text-[var(--color-text-muted)]">alíquota</span>
              </div>
              <button
                type="button"
                onClick={() => removeTax(index)}
                className="justify-self-end rounded-full border border-transparent px-3 py-1 text-xs font-semibold text-[var(--color-danger)] transition hover:border-[var(--color-danger)]"
              >
                Remover
              </button>
            </div>
          ...
          {value.taxes.length === 0 && (
            <p className="rounded-lg border border-dashed border-[var(--color-border)] px-3 py-4 text-sm text-[var(--color-text-muted)]">
      ...
      <div className="space-y-1">
        <label className="text-sm font-medium text-[var(--color-text)]" htmlFor="billing-conditions">
      ...
      <div className="space-y-1">
        <label className="text-sm font-medium text-[var(--color-text)]" htmlFor="billing-discount-policy">
