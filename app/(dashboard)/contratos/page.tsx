    ...
    <Fragment>
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <button
              className="rounded-full border border-[var(--color-accent)] px-5 py-2 text-sm font-semibold text-[var(--color-accent)] transition hover:border-[var(--color-accent-dark)] hover:text-[var(--color-accent-dark)]"
        ...
        {contracts.map((contract) => (
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[var(--color-text)]">{contract.code}</h2>
              <span className="rounded-full bg-[var(--color-accent-soft)] px-3 py-1 text-xs font-semibold text-[var(--color-accent)]">
                {contract.status}
              </span>
            </div>
            <p className="text-sm text-[var(--color-text-muted)]">Cliente: {contract.client.name}</p>
            <div className="rounded-lg bg-[var(--color-surface-muted)] p-4 text-sm text-[var(--color-text-muted)]">
              Vigência
            ...
            <div className="flex flex-wrap gap-2 text-xs text-[var(--color-text-muted)]">
              <span className="rounded-full border border-[var(--color-border)] px-3 py-1">Revisão jurídica em 7 dias</span>
              <span className="rounded-full border border-[var(--color-border)] px-3 py-1">Última atualização há 3 dias</span>
            </div>
          </article>
        ))}
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-[var(--color-text)]">Carregamento de cards</h2>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px,1fr]">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-semibold uppercase text-[var(--color-text-muted)]">
                    ...
                    <p className="text-xs text-[var(--color-text-muted)]">{contract.client.name}</p>
        ...
        <main className="space-y-5">
          {selectedContract ? (
            <>
              <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] p-6 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <div className="rounded-full bg-[var(--color-accent-soft)] px-4 py-1 text-xs font-semibold uppercase text-[var(--color-accent)]">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  ...
                  <div className="space-y-1">
                    ...
                    {selectedContract.highlights.length > 0 ? (
                      <ul className="list-disc space-y-1 pl-4 text-xs text-[var(--color-text-muted)]">
                        {selectedContract.highlights.map((highlight) => (
                          <li key={highlight}>{highlight}</li>
                        ))}
                      </ul>
                    ) : (
                      <p className="text-xs text-[var(--color-text-muted)]">Sem destaques registrados.</p>
                    )}
                  </div>
                </div>
              </section>
              ...
              <section className="grid gap-5 lg:grid-cols-2">
                <article className="flex flex-col gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] p-5 shadow-sm">
                  <header className="flex items-center justify-between">
                    <div>
                      <h3 className="text-base font-semibold text-[var(--color-text)]">Status de assinatura</h3>
                      <p className="text-xs text-[var(--color-text-muted)]">
                    ...
                    <button
                      ...
                      className="rounded-full border border-[var(--color-border)] px-3 py-1 text-xs font-semibold text-[var(--color-text)] transition hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]"
                    >
                      Editar
                    </button>
                  </header>
                  <div className="rounded-xl bg-[var(--color-surface-muted)] p-4 text-sm text-[var(--color-text-muted)]">
                    <p>
                ...
                <article className="flex flex-col gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] p-5 shadow-sm">
                  <header className="flex items-center justify-between">
                    <div>
                      <h3 className="text-base font-semibold text-[var(--color-text)]">Resumo financeiro</h3>
                      <p className="text-xs text-[var(--color-text-muted)]">Valores previstos e faturamento em andamento.</p>
                    </div>
                    <button
                      type="button"
              ...
              <section className="grid gap-5 lg:grid-cols-2">
                <article className="flex flex-col gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] p-5 shadow-sm">
                  <header className="flex items-center justify-between">
                    <h3 className="text-base font-semibold text-[var(--color-text)]">Template & documento</h3>
                    <button
                      type="button"
                  ...
                  <div className="flex flex-wrap gap-2 text-xs">
                    ...
                    {selectedContract.docConfig.sendForLegalReview && (
                      <span className="rounded-full bg-[var(--color-surface-muted)] px-3 py-1 font-semibold text-[var(--color-text)]">
                ...
                <article className="flex flex-col gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] p-5 shadow-sm">
                  <header className="flex items-center justify-between">
                    <h3 className="text-base font-semibold text-[var(--color-text)]">Cláusulas e garantias</h3>
                    <button
                      type="button"
              ...
              <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] p-5 shadow-sm">
                <header className="flex items-center justify-between">
                  <h3 className="text-base font-semibold text-[var(--color-text)]">Equipe envolvida</h3>
                  <button
                    type="button"
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <div className="space-y-2 text-sm text-[var(--color-text-muted)]">
                    <p>
                      <span className="font-semibold text-[var(--color-text)]">Responsável: </span>
                  ...
                  <div className="space-y-2">
                    ...
                    <ul className="space-y-2 text-sm text-[var(--color-text)]">
                      {uniqueTeamMembers.map((member) => (
                        <li key={member.id} className="rounded-lg border border-[var(--color-border)] px-3 py-2">
                          <p className="font-semibold">
                            {member.name}
                            <span className="ml-2 text-xs uppercase text-[var(--color-text-muted)]">{member.type}</span>
                          </p>
